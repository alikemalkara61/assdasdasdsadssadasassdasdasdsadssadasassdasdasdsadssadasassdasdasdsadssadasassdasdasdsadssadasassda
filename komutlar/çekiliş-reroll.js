...
exports.run = (client, message, args) => {
  if (!message.member.hasPermission("MANAGE_MESSAGES")) {
    return message.channel.send(
      " Yetersiz yetki! gereken yetki; `MESAJLARI YÖNET`."
    );
  }
  ...
  try {
    ...
    if (
      error.startsWith(
        ` ${messageID} IDsi ile başlayan bir çekiliş bulunamadı!.`
      )
    ) {
    ...
    if (
      error.startsWith(` ${messageID} IDsi ile başlayan çekiliş bitmemiş!.`)
    ) {
